...
public class HungarianAlgorithm {
	...
	public HungarianAlgorithm(Double[][] matrix) {
		...
		this.matrix = matrix;
		squareInRow = new int[matrix.length]; // squareInRow & squareInCol
												// indicate the position
		squareInCol = new int[matrix[0].length]; // of the marked zeroes

		rowIsCovered = new int[matrix.length]; // indicates whether a row is
												// covered
		colIsCovered = new int[matrix[0].length]; // indicates whether a column
													// is covered
	 ...
	 * find an optimal assignment
	 *
	 * @return Cost of optimal assignment
	 */
	public double findOptimalAssignmentCost() {
		step1(); // reduce matrix
		step2(); // mark independent zeroes
		step3(); // cover columns which contain a marked zero
